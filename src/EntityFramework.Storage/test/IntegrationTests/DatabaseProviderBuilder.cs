...
namespace ForgePoint.Identity.EntityFramework.IntegrationTests
{
    ...
    public class DatabaseProviderBuilder
    {
        ...
        {
            var builder = new DbContextOptionsBuilder<T>();
            builder.UseSqlite($"Filename=./Test.ForgePoint.Identity.EntityFramework-3.1.0.{name}.db");
            return builder.Options;
        }
        ...
        public static DbContextOptions<T> BuildLocalDb<T>(string name) where T : DbContext
        {
            ...
            builder.UseSqlServer(
                $@"Data Source=(LocalDb)\MSSQLLocalDB;database=Test.ForgePoint.Identity.EntityFramework-3.1.0.{name};trusted_connection=yes;");
            return builder.Options;
        }
    }
}