...
Implementation
^^^^^^^^^^^^^^
Our `quick start UI <https://github.com/joneja09/ForgePoint.Identity/tree/main/samples/Quickstarts>`_ utilizes some of the below features. Also check out the :ref:`external authentication quickstart <refExternalAuthenticationQuickstart>` and the 
